...
def config(targets, config, variables=None, output=None, sequential=False, password_prompt=False):
    if password_prompt:
        password = click.prompt('Please input the password', hide_input=True)
    else:
        password = None
    target_groups = get_target_groups(targets, password)
    var_tree = get_complete_variables(variables, target_groups) if variables else {}
    handler = ConfigHandler(logger, config)
    result = handler.execute_config(target_groups, out_file=output, var_tree=var_tree, sequential=sequential)
    with open("multi_exec.json", "w") as result_json:
        json.dump(result, result_json, indent=4)



def get_target_groups(targets_path, password):
